...
-export([test_add_handler/1, test_remove_handler/1, test_get_events/1,
         test_get_handlers/1, test_get_priority/1, test_set_priority/1,
         test_get_event_queue/1, test_prune_event_queue/1]).
...
test_set_priority(_Config) ->
    ...
    {error, priority_not_integer} =
        blockade:set_priority(test_set_priority, 1.5, #{}).
...
test_get_event_queue(_Config) ->
    ...
    {ok,
     [{test_event, [], #{priority := 9}}, {test_event, [], #{priority := 0}}]} =
...
test_prune_event_queue(_Config) ->
    ...
    blockade:dispatch(test_prune_event_queue,
                      test_event,
                      [],
                      #{priority => -1}),
