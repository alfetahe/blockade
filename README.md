...
When a process dies or is terminated, the process will be unsubscribed from the event queue automatically.
...
# If discard_events is set to true, events with lower priority than the current priority will be discarded immediately after dispatching the event.
...
# Wait 30 seconds and check the local mailbox again. There should be one event now.
> flush()
{:some_event_key, "priority_test"}

# Now let's try dispatching events with the priority option.

# Set priority to 150 and discard any queued events with lower priority.
# This time the event queue will keep the priority and will not reset back to 0 because we did not provide the reset_after option.
...
the event will be discarded if the priority level on the event queue is higher than the priority level on the dispatch call:
```elixir
...
:ok
> flush()
# The event will be discarded and won't be received by the handler.
```
...
- `priority` - set priority level for the event. Default is `0`. If the current priority on the event queue
...
- `discard_event` - if set to `true`, the event will be discarded if the priority level on the event queue is higher than the priority level on the dispatch call. Default is `false`.
- `atomic_priority_set` - if set, the priority level will be set atomically according to the passed value.
This is handy when we want to dispatch an event and immediately set the priority level to a higher value.
...
or dispatching events with the priority option.
...
> **Note**
> If dispatching events to all members across the cluster is not desired, you can start `blockade` with different names on different nodes. This way you can have multiple event queues within the cluster, each completely isolated from each other. For example, you can use the local node name as the event queue name, which is unique in the cluster.
